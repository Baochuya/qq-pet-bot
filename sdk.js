...
function dispatchEvent(listeners, event) {
  // 兼容 post_type（OneBot 口径）与 type（Manifest v2 口径）两种事件字段
  const { post_type, group_id } = event
  const ptype = post_type || event.type
  let key = null
  if (ptype === 'group_message')   key = 'group_message'
  if (ptype === 'friend_message')  key = 'friend_message'
  if (ptype === 'group_notice')    key = 'group_notice'
  if (ptype === 'friend_notice')   key = 'friend_notice'
  if (ptype === 'bot_offline')     key = 'bot_offline'
  if (key && listeners[key]) {
    try { listeners[key](event) } catch (e) { log.err('事件回调异常:', e) }
  }
}

// ========== createAPI（增强：url/自动重连/生命周期/命令注册） ==========
export function createAPI(config) {
  const {
    host = '127.0.0.1', port = 3001, url, token, name, version, author,
    autoReconnect = true, reconnectBaseMs = 3000, reconnectMaxMs = 60000,
    onLifecycle, onControlResult, onJobTrigger,
  } = config
  ...
  function call(action, params, resultMessage = '', resultData = true, timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        pending.delete(id)
        const error = `action ${action} 超时 (${Math.round(timeoutMs/1000)}s)`
  ...
  function controlRegister(payload, timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
      const id = String(++nextId)
      const timer = setTimeout(() => {
        waiting.delete(id)
        reject(new Error('control_register 超时'))
      }, timeoutMs)
      waiting.set(id, { resolve, reject, timer })
      _send({ type: 'control_register', id, ...payload })
    })
  }
  ...
  function handleMessage(msg) {
    switch (msg.type) {
      case 'auth_ok':
        ...
        log.ok('认证成功')
        startPing()
        if (resolveWaiters) resolveWaiters.ref()
        break
      case 'auth_failed':
        if (onLifecycle) onLifecycle({ event: 'auth_failed', message: msg.message })
        break
      case 'event':
      ...
      case 'action_result':
        if (msg.id && pending.has(msg.id)) {
          ...
          if (item.resultMessage) {
            ...
            else item.resolve({ code: 1, msg: msg.error || 'action failed' })
          }
          else if (msg.ok) item.resolve(msg.data)
          else item.reject(new Error(msg.error || 'action failed'))
        }
        break
      case 'lifecycle':
        if (onLifecycle) onLifecycle(msg)
        break
      case 'control_result':
        if (msg.id && waiting.has(msg.id)) {
          ...
          if (msg.ok) item.resolve(msg.result || msg)
          else item.resolve({ code: 1, msg: msg.error || 'control_register failed' })
        }
        break
      case 'job_trigger':
        if (onJobTrigger) onJobTrigger(msg)
        break
      case 'pong': break
    }
  }
  ...
  function connectOnce() {
    return new Promise((resolve, reject) => {
      ...
      ws.on('close', (code) => {
        ...
        pending.clear()
        for (const [, p] of waiting) { clearTimeout(p.timer); p.reject(new Error('连接断开')) }
        waiting.clear()
        stopPing()
        if (wasConnected) log.warn(`连接断开 code=${code}`)
        if (autoReconnect && !closing) scheduleReconnect()
      })
      ws.on('error', (err) => {
        // error 后一般会触发 close；这里避免重复 reject 造成未捕获异常
  ...
  function scheduleReconnect() {
    ...
    reconnectTimer = setTimeout(async () => {
      ...
      log.info(`重新连接 ${wsUrl}（第 ${attempt} 次，${Math.round(delay/1000)}s 后）`)
      setTimeout(async () => {
        try { await connectOnce() } catch {}
        if (!connected && autoReconnect && !closing) scheduleReconnect()
      }, delay)
    }, 300)
  }

  // resolveWaiters：connect() 只在首次成功时 resolve
  let resolveWaiters = null
  function connect() {
    log.info(`连接 ${wsUrl} ...`)
    closing = false
    if (connected) return Promise.resolve()
    if (resolveWaiters) return resolveWaiters.promise
    const p = new Promise((resolve, reject) => {
      resolveWaiters = { ref: resolve }
    })
    resolveWaiters.promise = p
    connectOnce()
    return p
  }
  ...
  const api = { on, connect, disconnect, controlRegister }
