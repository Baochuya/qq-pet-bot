...
export function createReverseAPI(config = {}) {
  const {
    host = '0.0.0.0', port = 3002, path = '/', token,
  ...
  function handleConnection(socket, req) {
    ...
    socket.on('message', data => {
      ...
      if (msg.type === 'ready') {
        ...
        if (msg.id && ctlPending.has(msg.id)) {
          const item = ctlPending.get(msg.id)
          clearTimeout(item.timer)
          ctlPending.delete(msg.id)
          if (msg.ok) item.resolve(msg.result || msg)
          else item.resolve({ code: 1, msg: msg.error || 'control_register failed' })
        }
        if (onControlResult) onControlResult(msg)
      } else if (msg.type === 'job_trigger') {
  ...
  function close() {
    ...
    ready = false
    rejectPending('服务已关闭')
    for (const [, item] of ctlPending) { clearTimeout(item.timer); item.reject(new Error('服务已关闭')) }
    ctlPending.clear()
    for (const waiter of connectionWaiters) {
      clearTimeout(waiter.timer)
  ...
  function controlRegister(payload, timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
      if (!ready || !ws || ws.readyState !== WebSocket.OPEN) {
        reject(new Error('萌卡NT 后台尚未建立反向 WebSocket 连接'))
        return
      }
      const id = String(++nextId)
      const timer = setTimeout(() => {
  ...
  const api = { on, listen, waitForConnection, close, controlRegister }
